function createEntry(entryType) {
    ...
    saveButton.addEventListener("click", function() {
        ...
        if (title && description) {
            ...
            // Append the checkboxes container to the entry
            entryBox.appendChild(checkboxesContainer);

            // Now add the "expand" button at the very bottom of the container (centered)
            const expandButton = document.createElement("button");
            expandButton.textContent = "▼"; // Down arrow initially
            expandButton.style.marginTop = "10px";
            expandButton.style.fontSize = "14px"; // Smaller size for subtlety
            expandButton.style.display = "block";
            expandButton.style.marginLeft = "auto";
            expandButton.style.marginRight = "auto";
            expandButton.style.background = "none";
            expandButton.style.border = "none";
            expandButton.style.cursor = "pointer";
            expandButton.style.color = "#888"; // Light grey for subtlety
            expandButton.style.position = "absolute"; // Position the expand button at the bottom
            expandButton.style.left = "50%";
            expandButton.style.transform = "translateX(-50%)"; // Center the button horizontally
            entryBox.style.position = "relative"; // Make sure the entryBox is positioned to hold the absolute button
            entryBox.appendChild(expandButton);

            // Add the collapsible description and goal section
            const descriptionLabelExpand = document.createElement("label");
            descriptionLabelExpand.textContent = "Description:";
            descriptionLabelExpand.style.fontSize = "12px"; // Small font size to match
            descriptionLabelExpand.style.fontWeight = "bold"; // Bold style like the title
            descriptionLabelExpand.style.marginBottom = "5px"; // Add a little space between label and input
            descriptionLabelExpand.style.width = "100%";
            descriptionLabelExpand.style.marginBottom = "10px";

            const goalLabelExpand = document.createElement("label");
            goalLabelExpand.textContent = "What goal does this habit support?";
            goalLabelExpand.style.fontSize = "12px"; // Small font size to match
            goalLabelExpand.style.fontWeight = "bold"; // Bold style like the title
            goalLabelExpand.style.marginBottom = "5px"; // Add a little space between label and input
            goalLabelExpand.style.width = "100%";
            goalLabelExpand.style.marginBottom = "10px";

            const descriptionElement = document.createElement("p");
            descriptionElement.textContent = `${description}`;
            const goalElement = document.createElement("p");
            goalElement.textContent = `${selectedGoal}`;
            entryContent.appendChild(descriptionLabelExpand);
            entryContent.appendChild(descriptionElement);
            entryContent.appendChild(goalLabelExpand);
            entryContent.appendChild(goalElement);

            // Set the expand/collapse button's action
            let isExpanded = false;
            expandButton.addEventListener("click", function() {
                if (isExpanded) {
                    entryContent.style.display = "none"; // Hide extra content
                    expandButton.textContent = "▼"; // Change arrow to expand
                } else {
                    entryContent.style.display = "block"; // Show extra content
                    expandButton.textContent = "▲"; // Change arrow to collapse
                }
                isExpanded = !isExpanded; // Toggle state
            });

            // Add the extra content to the entry
